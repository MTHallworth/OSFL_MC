---
title: "OSFL geolocator analysis"
...
library(knitr)
opts_chunk$set(warning = FALSE, tidy = TRUE)
```

```{r packages, include=FALSE, message = FALSE, waring = FALSE}
.libPaths("Packages")
...
# Create a path to hold the temporary raster information
rasterOptions(tmpdir="temporary_raster_folder")
```


```{r SpatialData, warning = FALSE, echo = FALSE}
...
### Take the log of the light values
...
The seed is a time when we know the geolocator recorded no light (i.e., when it's known to be dark). See the image below. The dark areas represent times when the light level recorded by the geolocator fell below 1. The red dot is the time specified when we know it's dark. This helps the function automatically define the transitions.    
...
# Save year K288's second year separately
twl[[16]] <- twl[[4]][595:nrow(twl[[4]]),]

# Keep only the first year for K288 
twl[[4]] <- twl[[4]][1:594,]
```

```{r twlight_example, echo = FALSE}
...
twl[[i]] <- twilightEdit(twilights = twl[[i]], 
                           ...
                           stationary.mins = 25, # edit if consecutive twilights (window) within x mins
                           plot = FALSE)
}

# Keep track of the threshold used to assign twilights
twl <- Map(cbind, twl, threshold = 1)
```

### Higher threshold to extend into arctic 

```{r}
# Create an empty list to store the results
twl_Ext <- vector('list',(nBirds+1))
...
# Keep track of threshold used to determine twilights
twl_Ext <- Map(cbind, twl_Ext, threshold = 4)
```

## Combine the data sets

here we combine the data sets 
```{r}
## ------------------------------------------------------------------------
...
```{r exampleTWlEdit, echo = FALSE, fig.cap= "Red dots = Sunset, blue = Sunrise, arrows indicate the transtions that were moved and X represents transitions that were removed."}
...
All transition data are stored including information of the transitions that were modified or deleted. The original transitions are also stored (twilight0). 
...
### Adjust the sunset times and remove transitions labeled deleted
```{r adjusttime}
...
```{r echo = FALSE, fig.cap = "Example of light data with associated twilight events - red = sunset, blue = sunrise", dpi = 300}
lightImage(OSFLdata[[5]], zlim = c(0,5),offset = 19)
tsimagePoints(twlEdit[[5]]$Twilight, 
              offset = 12, 
              pch = 16, 
              cex = 0.5,
              col = ifelse(twlEdit[[5]]$Rise, "dodgerblue", "firebrick"))
```
...
### Determining zenith angles
Here we only use the anchorage birds because they experience light/dark periods. Birds farther north don't experience light/dark period when they are known to be on breeding territories. 
...
```{r plot1, echo = FALSE, fig.cap = "*Left* The deviation in twilights from the true twilight at the capture site. *Right* The mean (point estimate) and 95% CI for the zenith angle at the capture location."}
...
```
### Set tolerance value around the equinox period.     
These values don't change longitude estimates, only latitude. They interpolate latitude around equniox periods The larger the tol value the longer the interpolation. The edits below were make manually to best match migration through Central America. 
```{r setTols, eval = FALSE}
...
### Obtain the intial path based on a simple threshold analysis. 
```{r estPath, eval = FALSE}
...
```{r pathsPlot, eval = FALSE, echo = FALSE}
...
# Set the dates when we know the positions are fixed - i.e, at the capture site. 
for(i in 1:3){
...
### Define mask to allow OSFL to stop only on land

```{r distmask, eval = FALSE}
...
## Determine the locations in a MCMC framework using SGAT. 
```{r eval = FALSE}
...
```{r readFit, echo = FALSE, eval = FALSE}
...
$$Importance = \frac{\left(\sum(Stop Duration) * \frac{BirdsUsed}{max(BirdsUsed)} \right )}{max(\sum(Stop Duration) * \frac{BirdsUsed}{max(BirdsUsed)})}$$
