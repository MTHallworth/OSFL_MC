...
### Keeping track of the threshold used to assign twilights
```{r}
twl <- Map(cbind, twl, threshold = 1)
twl_Ext <- Map(cbind, twl_Ext, threshold = 4)
``` 

here we combine the datasets 
```{r}
...
fit <- readRDS("Data/MCMC_fit/OSFL_fit_Jan_27_2017.rds")
str(fit[[1]])
```
...
source("Functions/MigSchedules.R")

schedules <- vector('list',(nBirds+1))

for(i in 1:(nBirds+1)){
schedules[[i]]<- MigSchedule(MCMC = S[[i]], 
                             prob = 0.95, 
                             known.stationary = c(Nov1[i],Mar1[i]),
                             make.stationary.raster = TRUE, 
                             plot.legend = FALSE,
                             rm.lat.equinox = TRUE, 
                             days.omit = 5)
}
...
$$Importance = \frac{\left(\sum(Stop Duration) * BirdsUsed \right )}{max(\sum(Stop Duration) * BirdsUsed)}$$

```{r}
SpringImportance <- sum(stack(springstops))*sum(birdsspring) / 
                    cellStats(sum(stack(springstops))*sum(birdsspring),max)

FallImportance <- sum(stack(fallstops))*sum(birdsfall) / 
                  cellStats(sum(stack(fallstops))*sum(birdsfall),max)
```
